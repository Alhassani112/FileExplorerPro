...
class FileRepository(private val ctx: Context) {
    ...
    suspend fun listDirectory(dirUri: Uri, showHidden: Boolean): List<FileItem> = withContext(Dispatchers.IO) {
        ...
        dir.listFiles().mapNotNull { f ->
            ...
            FileItem(f.uri, n, f.isDirectory, if (f.isDirectory) 0 else f.length(), f.lastModified(), f.type)
        }.map { if (it.isDirectory) it.copy(posterUri = PosterResolver.findPoster(ctx, it.uri)) else it }
    }
    suspend fun getStorageInfo(): StorageInfo = withContext(Dispatchers.IO) {
        ...
        } catch (e: Exception) { StorageInfo(0, 0, 0) }
    }
    ...
    suspend fun createFolder(p: Uri, name: String) = withContext(Dispatchers.IO) {
    ...
    suspend fun copyRecursive(s: FileItem, destUri: Uri): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            if (!s.isDirectory) {
                ...
                val nf = dest.createFile(src.type ?: "application/octet-stream", s.name) ?: return@withContext false
                ctx.contentResolver.openInputStream(s.uri)?.use { i ->
                    ctx.contentResolver.openOutputStream(nf.uri)?.use { o -> i.copyTo(o, 65536) }
                }
            ...
            for (c in src.listFiles()) {
                copyRecursive(FileItem(c.uri, c.name ?: "?", c.isDirectory, c.length(), c.lastModified(), c.type), nd.uri)
            }
            true
        } catch (e: Exception) { false }
    }
    suspend fun moveRecursive(s: FileItem, destUri: Uri) =
